function result = RL_5P(choice, reward, trialtype)
...
for i = 1:10 % 10 iterations
    ...
    result.iter(i).inV = b(5); %initial value
    result.iter(i).inV_se = se(5); %initial value se
...
V(1,:) = [inV inV inV]; %fitting start value
rpe = zeros(ntrial,1);
cV = zeros(ntrial,1);
