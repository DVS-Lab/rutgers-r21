...
outdir = fullfile(maindir,'TrialData_4Pmodel');
if ~exist(outdir,'dir')
    mkdir(outdir);
end

fid_summary = fopen(fullfile(maindir,'summary_4P_final2.csv'),'w');
fprintf(fid_summary,'subject,condition,alpha1,alpha1_se,alpha2,alpha2_se,alpha3,alpha3_se,beta,beta_se,psuedoR2,BIC\n');
for s = 1:length(subjects)
    ...
    for c = 1:length(conditions)
        ...
        result = RL_4P(SlotChoice, Reward, TrialType);
        
        %fprintf(fid_summary,'subject,condition,alpha,alpha_se,beta,beta_se,psuedoR2,BIC\n');
        R = result.final;
        fprintf(fid_summary,'%d,%s,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f\n',subject,condition,R.alpha1,R.alpha1_se,R.alpha2,R.alpha2_se,R.alpha3,R.alpha3_se,R.beta,R.beta_se,R.pseudoR2,R.BIC);
        
        fid_subj = fopen(fullfile(outdir,['rpe_' num2str(subject) '_' condition '_4P.csv']),'w');
