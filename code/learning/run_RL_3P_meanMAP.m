...
outdir = fullfile(maindir,'TrialData_3Pmodel_meanMAP');
if ~exist(outdir,'dir')
    mkdir(outdir);
end
...
for s = 1:length(subjects)
    ...
    for c = 1:length(conditions)
        ...
        result = RL_3P_meanMAP(SlotChoice, Reward, TrialType);
        
        %fprintf(fid_summary,'subject,condition,alpha,alpha_se,beta,beta_se,psuedoR2,BIC\n');
        R = result;
        
        fid_subj = fopen(fullfile(outdir,['rpe_' num2str(subject) '_' condition '_3P_meanMAP.csv']),'w');
